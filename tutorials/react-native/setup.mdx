...
The CoCart TypeScript SDK works in React Native out of the box since it uses native `fetch`. Create `src/api/cocart.ts`:

```typescript
import { CoCart } from '@cocartheadless/sdk';
import Config from 'react-native-config';

const STORE_URL = Config.STORE_URL || 'https://yourstore.com';

let client: CoCart | null = null;

export function getClient() {
  if (!client) {
    client = new CoCart(STORE_URL);
  }
  return client;
}
```
...
<Tip>
  The SDK handles cart key tracking, authentication, and request formatting automatically. See the [SDK documentation](/sdk/typescript) for the full API including JWT authentication.
</Tip>
...
import { getClient } from '../api/cocart';
...
export function useCart() {
  ...
  const updateCartMutation = useMutation({
    mutationFn: ({ itemKey, quantity }: any) =>
      client.cart().updateItem(itemKey, { quantity }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cart'] });
    },
    onError: (error: any) => {
  ...
  const removeCartMutation = useMutation({
    mutationFn: (itemKey: string) => client.cart().removeItem(itemKey),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cart'] });
    },
    onError: (error: any) => {
...
export function useProduct(productId: string | number) {
  ...
  return useQuery({
    ...
    queryFn: () => client.products().find(productId),
    enabled: !!productId,
  });
}
```
...
If you encounter CORS errors, configure WordPress to allow cross-origin requests. See [CORS documentation](/cors).
