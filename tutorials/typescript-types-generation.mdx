...
description: "Generate TypeScript types from CoCart's OpenAPI specs for type-safe fetch, Axios, or React Query integrations — without using the SDK."
icon: "code"
tag: "NEW"
---

<Info>
  This tutorial was written by [Claude Code (an AI)](https://claude.com/product/claude-code). Last updated: July 29th, 2026.
</Info>

<Info>
  **Using `@cocartheadless/sdk`?** TypeScript types are already included — no generation step needed. This tutorial is for developers using raw `fetch()`, Axios, or React Query directly. [See the TypeScript SDK docs →](/sdk/typescript)
</Info>

TypeScript provides strong typing that helps catch errors at compile time and improves developer experience with better autocompletion. CoCart publishes official OpenAPI 3.0.3 specification files in the [`cocart-headless/cocart-openapi`](https://github.com/cocart-headless/cocart-openapi) repository that you can use to automatically generate TypeScript types for all API endpoints, request bodies, and response schemas.

This tutorial shows you how to generate and use those types in your TypeScript projects.
...
All official specs live in [`cocart-headless/cocart-openapi`](https://github.com/cocart-headless/cocart-openapi) and follow the OpenAPI 3.0.3 standard.

### CoCart Starter — API v2 (latest, v5.0.0)

| Specification File | Covers |
|-------------------|--------|
| `starter/v2/openapi-cocart-v2-collection.yaml` | **All v2 endpoints combined** — cart, products, sessions, store, login/logout |
| `starter/v2/openapi-cocart-cart-v2.yaml` | Cart endpoints only |
| `starter/v2/openapi-cocart-products-v2.yaml` | Products endpoints only |
| `starter/v2/openapi-cocart-sessions-v2.yaml` | Sessions endpoints only |
| `starter/v2/openapi-cocart-others-v2.yaml` | Login, logout, and store endpoints |

<Note>
  `starter/v2/openapi-cocart-v2-sdk.yaml` also exists in the repo, but it's the spec used to generate the official SDKs' own code samples (it carries `x-codeSamples` for the TypeScript/PHP/Python/Go SDKs and formats example values to match SDK output). Use the collection spec above for generic type generation instead.
</Note>

### CoCart Community — API v2 (v4.9.0)

| Specification File | Covers |
|-------------------|--------|
| `community/v2/openapi-cocart-v2-community.yaml` | All community v2 endpoints combined |
| `community/v2/openapi-cocart-cart-v2.yaml` | Cart endpoints only |
| `community/v2/openapi-cocart-products-v2.yaml` | Products endpoints only |
| `community/v2/openapi-cocart-sessions-v2.yaml` | Sessions endpoints only |
| `community/v2/openapi-cocart-others-v2.yaml` | Login, logout, and store endpoints |

### CoCart Community — API v1 (legacy)

<Note>
  No longer supported. Only use these if you're stuck on an old CoCart version that hasn't migrated to v2.
</Note>

| Specification File | Covers |
|-------------------|--------|
| `community/v1/openapi-cocart-cart-v1.yaml` | Legacy v1 cart endpoints |
| `community/v1/openapi-cocart-products-v1.yaml` | Legacy v1 product endpoints |

### Add-ons

| Specification File | Covers |
|-------------------|--------|
| `jwt-authentication/openapi-cocart-jwt-auth.yaml` | JWT token refresh and validation |
| `plus/v1/openapi-cocart-plus-v1-collection.yaml` | CoCart Plus v1 — coupons, fees, shipping, payment methods, and granular totals |
| `plus/v2/openapi-cocart-plus-v2-collection.yaml` | CoCart Plus v2 (in development) — cart, products, my account (excludes checkout, not yet published) |

<Note>
  This tutorial focuses on **CoCart Starter API v2** (the collection spec), which is recommended for all new projects. The same approach works with any of the specs above.
</Note>
...
```bash
npx openapi-typescript https://raw.githubusercontent.com/cocart-headless/cocart-openapi/main/starter/v2/openapi-cocart-v2-collection.yaml -o src/types/cocart.ts
```
...
{
  "scripts": {
    "generate-types": "openapi-typescript https://raw.githubusercontent.com/cocart-headless/cocart-openapi/main/starter/v2/openapi-cocart-v2-collection.yaml -o src/types/cocart.ts"
  }
}
```
...
```bash
curl -o openapi-cocart-v2.yaml https://raw.githubusercontent.com/cocart-headless/cocart-openapi/main/starter/v2/openapi-cocart-v2-collection.yaml
```

2. **Generate types from the local file**:

```bash
npx openapi-typescript ./openapi-cocart-v2.yaml -o src/types/cocart.ts
```
...
npx openapi-typescript https://raw.githubusercontent.com/cocart-headless/cocart-openapi/main/starter/v2/openapi-cocart-v2-collection.yaml \
...
### Generating Types for Multiple Specs

You can generate types for different parts of the API separately, or include add-on specs like JWT:

```json
{
  "scripts": {
    "generate-types:v2": "openapi-typescript https://raw.githubusercontent.com/cocart-headless/cocart-openapi/main/starter/v2/openapi-cocart-v2-collection.yaml -o src/types/cocart-v2.ts",
    "generate-types:jwt": "openapi-typescript https://raw.githubusercontent.com/cocart-headless/cocart-openapi/main/jwt-authentication/openapi-cocart-jwt-auth.yaml -o src/types/cocart-jwt.ts",
    "generate-types:plus": "openapi-typescript https://raw.githubusercontent.com/cocart-headless/cocart-openapi/main/plus/v1/openapi-cocart-plus-v1-collection.yaml -o src/types/cocart-plus.ts",
    "generate-types:all": "npm run generate-types:v2 && npm run generate-types:jwt && npm run generate-types:plus"
  }
}
```

You can then import from each generated file as needed:

```typescript
import type { paths as CartPaths } from './types/cocart-v2';
import type { paths as JwtPaths } from './types/cocart-jwt';
```
...
2. [Add user authentication](/authentication) with typed requests
3. [Build a checkout flow](/api-reference/v2/cart/post-add-item) with full type coverage
4. [Explore the API Reference](/api-reference) to see all available endpoints

## Resources

<CardGroup cols={2}>
  <Card title="CoCart OpenAPI Specs" icon="github" href="https://github.com/cocart-headless/cocart-openapi">
    Official OpenAPI specification files for all CoCart editions
  </Card>
  ...
  <Card title="CoCart API Reference" icon="code" href="/api-reference">
