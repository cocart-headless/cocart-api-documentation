...
The CoCart TypeScript SDK includes a Svelte adapter. Create `src/lib/cocart.ts`:
...
export function createClient() {
  return new CoCart(PUBLIC_STORE_URL || 'https://yourstore.com');
}
```

<Tip>
  The SDK handles cart key persistence, authentication, and request formatting automatically. See the [SDK documentation](/sdk/typescript) for the full API.
</Tip>
...
```typescript
import { createClient } from '$lib/cocart';
...
export const actions = {
  addToCart: async ({ request }) => {
    ...
    try {
      const client = createClient();
      await client.cart().addItem(productId as string, { quantity });
      return { success: true };
    } catch (error) {
...
If you encounter CORS errors, you may need to configure WordPress to allow cross-origin requests. See the [CORS documentation](/cors).
