...
sidebarTitle: "Mollie"
description: "Complete guide to integrating Mollie Components with Checkout API"
icon: "credit-card"
tag: "WIP"
...
<AccordionGroup>
    <Accordion title="Mollie.js Not Loading">
        **Problem**: Mollie object is undefined.

        **Solution**:
        ```javascript
        // Verify Mollie is loaded
        if (typeof Mollie === 'undefined') {
            console.error('Mollie.js not loaded. Check script tag.');
        }
    ...
    <Accordion title="Components Not Mounting">
        **Problem**: Card fields don't appear.

        **Solution**:
        ```javascript
        // Check profile ID is valid
        const mollie = Mollie(profileId, { testmode: true });
        ...
        cardNumber.mount('#card-number').catch(error => {
            console.error('Mount error:', error);
        });
        ```
    </Accordion>

    <Accordion title="Token Creation Fails">
        **Problem**: createToken() returns errors.

        **Solution**:
        ```javascript
        mollie.createToken().then(result => {
            if (result.error) {
                console.error('Token error:', result.error);
                // Common: invalid card, incomplete fields
            } else {
    ...
    <Accordion title="Payment Method Not Recognized">
        **Problem**: Backend doesn't accept Mollie payment.

        **Solution**:
        ```javascript
        const checkoutData = {
            payment_method: 'mollie_wc_gateway_creditcard', // Correct ID
            payment_data: {
                token: result.token,
    ...
    <Accordion title="Test Mode vs Live Mode Issues">
        ...
        **Solution**:
        ...
        // For live mode
        const mollie = Mollie(profileId, { testmode: false });
        ```
        Checklist:
...
```javascript
if (DEBUG) {
    console.log('[Mollie] Profile ID:', profileId);
    console.log('[Mollie] Test mode:', testmode);

    cardNumber.addEventListener('change', (event) => {
        console.log('[Mollie] Card valid:', event.valid);
        console.log('[Mollie] Errors:', event.error);
    });
}
```
...
1. **Mollie Documentation**: [Mollie Components Guide](https://docs.mollie.com/components/overview)
