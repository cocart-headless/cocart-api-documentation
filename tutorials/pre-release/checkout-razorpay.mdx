...
description: "Complete guide to integrating Razorpay Standard Checkout with Checkout API"
icon: "credit-card"
tag: "WIP"
...
<AccordionGroup>
    <Accordion title="Razorpay Checkout Script Not Loading">
        **Problem**: Razorpay object is undefined.
    ...
    <Accordion title="Checkout Modal Not Opening">
        **Problem**: rzp.open() doesn't show modal.
    ...
    <Accordion title="Payment Fails with Error Codes">
        **Problem**: Getting Razorpay error codes.
    ...
    <Accordion title="Payment Method Not Recognized">
        **Problem**: Backend doesn't accept Razorpay.

        **Solution**:
        ```javascript
        const checkoutData = {
            payment_method: 'razorpay', // Correct ID
            payment_data: {
                razorpay_payment_id: response.razorpay_payment_id,
                razorpay_order_id: response.razorpay_order_id,
                razorpay_signature: response.razorpay_signature
    ...
    <Accordion title="Test Mode vs Live Mode">
        **Problem**: Works in test but not live.

        **Solution**:
        ```javascript
        // Test mode: use test API key (starts with rzp_test_)
        // Live mode: use live API key (starts with rzp_live_)

        const options = {
            key: isTestMode ? 'rzp_test_xxx' : 'rzp_live_xxx',
            // ...
        };
        ```
        Checklist:
    ...
    <Accordion title="UPI Payment Issues">
        **Problem**: UPI payments failing.
...
1. **Razorpay Documentation**: [Razorpay Checkout Docs](https://razorpay.com/docs/payments/payment-gateway/web-integration/)
