...
```php Callback Class Example [expandable]
<?php
/**
 ...
 */
class My_Custom_CoCart_Callback extends CoCart_REST_Callback  {
	...
	public function callback( $request, $controller ) {
		try {
			...
			if ( $cart_updated ) {
				// $controller->calculate_totals( $request, true ); // Uncomment if you need to recalculate the cart totals.
...
```php
add_filter( 'cocart_rest_callbacks', 'register_my_custom_callback' );

function register_my_custom_callback( $callbacks ) {
	include_once( dirname( __FILE__) . '/callback/my-custom-callback.php' );

	$callbacks[] = new My_Custom_CoCart_Callback();
