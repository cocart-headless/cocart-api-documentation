...
(grep -rnE "apply_filters" "$CLONE_DIR" --include="*.php") | while IFS= read -r match; do
  FILTER_NAME=$(echo "$match" | sed -nE "s/.*['\"](cocart_[a-zA-Z0-9_\-]+)['\"].*/\1/p")

  # Skip if filter name doesn't match cocart_ prefix
  if [[ -z "$FILTER_NAME" || ! "$FILTER_NAME" =~ ^cocart_ ]]; then
