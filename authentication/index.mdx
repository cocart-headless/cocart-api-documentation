...
<Info>
  Using an official SDK? Credentials and Base64 encoding are handled automatically. See the [TypeScript](/sdk/typescript), [PHP](/sdk/php), [Python](/sdk/python), or [Go](/sdk/go) SDK docs to get started.
</Info>
...
<Tabs>
  <Tab title="TypeScript">
    ```typescript
    // After login, pass the JWT token directly or let the SDK manage it
    const client = new CoCart('https://your-store.com', {
      jwtToken: 'jwt_token',
    });
  ...
  <Tab title="Python">
    ```python
    client = CoCart('https://your-store.com', jwt_token='jwt_token')
...
## Transferring a Guest Cart on Login

If a customer begins shopping as a guest, you can transfer their cart items when they authenticate. See [Guest to Customer Cart](/knowledge-base/guest-to-customer-cart) for the process.
...
<Tabs>
  ...
  <Tab title="Go">
    ```go
    client := cocart.NewClient("https://your-store.com",
        cocart.WithWooCommerceKeys("ck_XXXXXXXXXXXXXXXXXXXXXX", "cs_XXXXXXXXXXXXXXXXXXXXXX"),
    )

    ctx := context.Background()
    sessions, err := client.Sessions().All(ctx)
    ```
  </Tab>
</Tabs>
...
<Note>
  When using an official CoCart SDK, Base64 encoding is handled automatically — you never need to encode credentials manually.
</Note>
...
<Note>
  CoCart SDKs always send credentials via the Authorization header. Query parameter fallback is not supported in the SDKs — if your server requires it, use raw HTTP requests instead.
</Note>
