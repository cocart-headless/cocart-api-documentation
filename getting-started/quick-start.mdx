---
title: "Quick Start"
description: "Learn how to set up a headless cart with CoCart — fetch products, add items, track the cart key, and load the cart at checkout."
icon: "rabbit-running"
tag: "Updated"
keywords: ["quick start", "cart", "headless", "add to cart", "cart key", "guest cart", "WooCommerce REST API", "CoCart"]
---

This tutorial uses a REST client for the examples, we will walk through the following steps in order to achieve placing items in the cart. The screenshots are from [Yaak](https://yaak.app), but you can also use other REST clients like [Postman](https://www.postman.com/) or [Insomnia](https://insomnia.rest/) if you want.

<Note>
    We now have SDK's available for **TypeScript/JavaScript**, **PHP**, **Python** and **Go**. These SDK's provide a more convenient way to interact with the CoCart API, abstracting away the complexities of making raw HTTP requests, handling authentication, and handling **cart management** and **customer information updates**. They also include helpful features like error handling, data validation.

    If you prefer using an SDK, you can find more information about them on our [SDKs](/sdk) page.
</Note>
...
Once the customer is ready to checkout, you can transfer the cart session to your WooCommerce store using the **Load Cart** feature. This is handy if you haven't yet integrated a headless payment solution like Stripe and want to fall back on the native WooCommerce checkout.

Append the `cocart-load-cart` query parameter with the cart key to any page on your store — the cart will be loaded in the background when the customer lands there:

```
https://your-store.com/checkout/?cocart-load-cart=<cart_key>
```

By default this will override any existing cart on the web store. You can set `keep-cart=false` to merge the cart items instead, and `notify=true` to display a notice to the customer on arrival.

<Note>
    Customer details are **not** transferred — only the cart items. This feature is not part of the REST API.
</Note>

For full details and filter options, see the [Load Cart](/load-cart) documentation.
...
- [Guiding principles](/developers/guiding-principles)
