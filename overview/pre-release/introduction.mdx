...
<Warning>
    **CoCart Starter** is still in development but feel free to review the documentation for it and provide feedback on changes, improvements and new features. **Beta releases will be out soon**.
</Warning>
...
<AccordionGroup>
    ...
    <Accordion title="Domain Dominance" icon="globe">
        Addressing CORS issues in decoupled setups, CoCart grants you control over the origin to address any CORS issues when decoupling. [View CORS section](/cors) for more info.
    </Accordion>
    ...
    <Accordion title="Bulk Cart Requests" icon="layer-group">
        Combine multiple API calls into one for better performance. [View Bulk Requests section](/batch-requests) for more info
    </Accordion>
</AccordionGroup>
...
The official TypeScript SDK is available on npm. Zero dependencies, typed responses, JWT support, and framework adapters for Astro and Next.js.
...
<Warning>
    The PHP, Python, and Go SDKs are still in development. In the mean time please refer to our [API Reference](/api-reference/pre-release/introduction) for direct API calls.
</Warning>
...
Ready to take your WooCommerce store to the next level? [Install CoCart now](/getting-started/setup) and start building the modern storefront you've always envisioned.
