...
description: "Handle errors from the CoCart Python SDK — exception hierarchy, catching exceptions, JWT token expiry, version guards, and HTTP status code mapping."
'og:title': "Error Handling in the CoCart Python SDK"
'og:description': "Handle CoCart REST API errors in Python. Exception hierarchy, catch by class, detect expired JWT tokens, VersionException for legacy plugin mode, and client-side ValueError validation."
---
...
See [Authentication](/sdk/python/authentication#auto-refresh) for details.
...
See [Authentication](/sdk/python/authentication#two-factor-authentication-2fa) for the full 2FA login flow.
