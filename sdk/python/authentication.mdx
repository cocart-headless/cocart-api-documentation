...
description: "Authenticate with CoCart using the Python SDK — guest sessions, Basic Auth, JWT, consumer keys, and custom auth headers."
'og:title': "Authentication with the CoCart Python SDK"
'og:description': "Authenticate headless WooCommerce from Python. Guest cart sessions, Basic Auth, JWT login with auto-refresh, consumer keys for admin, file-backed token persistence, and mock testing."
---
...
If the [CoCart 2FA](https://cocartapi.com) plugin is installed and the user has 2FA enabled, `login()` raises `TwoFactorRequiredException` instead of returning tokens. Catch it, prompt the user for a verification code, then call `verify_two_factor()` to complete the login:
