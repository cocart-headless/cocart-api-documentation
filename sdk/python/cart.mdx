---
title: "Python SDK: Cart API"
sidebarTitle: "Cart API"
description: "Manage shopping carts with the CoCart Python SDK — add, update, remove items, coupons, customer details, shipping, fees, and ETag caching."
'og:title': "Headless WooCommerce Cart API — CoCart Python SDK"
'og:description': "Manage WooCommerce carts from Python. Add and update items, apply coupons, set shipping, add fees, update customer details, handle guest and customer sessions, and use ETags."
---
...
`add_items()` is for adding multiple children of a single WooCommerce Grouped Product in one request — not for adding several unrelated products. For that, use `client.batch()` instead (requires CoCart Plus).
...
<Note>
  There's no real bulk-update endpoint on the server, so `update_items()` sends one request per item, sequentially, and returns the response from the last update (reflecting the fully-updated cart).
</Note>
...
For a true single round trip (requires CoCart Plus), use `batch_update_items()` instead — it builds the same entries into a `client.batch()` call:
...
<Note>
  Like `update_items()`, `remove_items()` sends one request per item key, sequentially, and returns the response from the last removal.
</Note>
...
For a true single round trip (requires CoCart Plus), use `batch_remove_items()` instead:
...
<Note>
  `update_customer()` sends billing fields unprefixed (`first_name`, `address_1`, ...) and shipping fields `s_`-prefixed (`s_first_name`, `s_address_1`, ...). If `shipping` is omitted or empty, billing is mirrored into the `s_` fields automatically — same as leaving "ship to a different address" unchecked at a normal WooCommerce checkout.
</Note>
...
Select a shipping rate for a package. Pass a `package_id` to restrict the selection to one package; omit it to apply the rate to every package.
...
<Warning>
  There is no address-taking shipping-calculation endpoint in the CoCart REST API. `calculate_shipping()` is deprecated — it ignores its `address` argument and simply delegates to `calculate()`. To calculate shipping for a destination, call `update_customer()` with that address first (the server recalculates totals as part of that request), then `calculate()` directly.
</Warning>
...
`client.batch()` dispatches multiple sub-requests in a single round trip via the `{namespace}/batch` endpoint. It returns one merged, up-to-date cart response with per-operation notices, instead of one response per request. `batch_update_items()` and `batch_remove_items()` (see above) build their requests through this method — call it directly for anything else you want to batch, e.g. mixing an add, a coupon, and a fee in one request:
...
See [Error Handling](/sdk/python/error-handling) for handling API errors.
