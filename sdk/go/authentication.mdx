...
<Note>Requires the [WordPress Two Factor plugin](https://wordpress.org/plugins/two-factor/), alongside CoCart Plus v1.6.0+ or CoCart Community v4.8+.</Note>

If a user has 2FA enabled, the server returns a `401` challenge response on the first login attempt instead of tokens. The SDK surfaces this as a `*TwoFactorRequiredError`, which you catch and handle before completing login with the OTP code.
