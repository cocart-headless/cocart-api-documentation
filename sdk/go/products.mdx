---
title: "Go SDK: Products API"
sidebarTitle: "Products API"
description: "Browse your WooCommerce catalog with the CoCart Go SDK — list, filter, search, paginate products, variations, categories, attributes, brands, and reviews."
'og:title': "WooCommerce Products API — CoCart Go SDK"
'og:description': "Query the WooCommerce product catalog from Go. Typed ProductListParams, range-over-func paginator (Go 1.23+), fetch variations, slug-based attribute lookups, brands, and reviews."
---
...
All list methods accept an optional `*ProductListParams` struct:
...
`AllPaginated()` returns a `*Paginator` that you can iterate with Go 1.23's range-over-func. It automatically fetches the next page when the current one is done:
