---
title: "Flutter SDK: Cart API"
sidebarTitle: "Cart API"
description: "Manage shopping carts with the CoCart Flutter SDK — add, update, remove items, coupons, customer details, shipping, fees, and ETag caching."
'og:title': "Headless WooCommerce Cart API — CoCart Flutter SDK"
'og:description': "Manage WooCommerce carts from Flutter. Client-side validation before network calls, add/update/remove items, coupons, shipping, fees, and ETag caching."
---
...
<Note>In legacy mode, this sends `fields` instead of `_fields`.</Note>
...
<Warning>
  `calculateShipping()` is deprecated: there is no address-taking shipping-calculation endpoint in the CoCart REST API, so this method now ignores its argument and just delegates to `calculate()`. Call `updateCustomer()` with the destination address to trigger server-side shipping recalculation, or call `calculate()` directly.
</Warning>
