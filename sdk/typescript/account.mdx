---
title: "TypeScript SDK: Account API"
sidebarTitle: "Account API"
description: "Access customer profiles, order history, downloads, and reviews with the CoCart TypeScript SDK Account API."
'og:title': "Customer Account API — CoCart TypeScript SDK"
'og:description': "Read and update a logged-in customer's profile, list and fetch orders (including guest order lookup), downloadable files, and submitted reviews from TypeScript."
---
...
The account endpoint gives authenticated customers access to their profile, order history, downloads, and reviews. All methods require the customer to be logged in via Basic Auth or JWT — guest access is not supported (except the dedicated guest order/download lookups below).

Access the account endpoint via `client.account()`. The instance is lazy-loaded on first call and reused thereafter.

<Note>
  The account API is provided by the CoCart Plus plugin. If the plugin is not installed, any account method throws a `CoCartError` with code `cocart_plugin_required` instead of a network error.
</Note>
...
See [Authentication](/sdk/typescript/authentication) for the full guide including JWT auto-refresh.
...
See [Error Handling](/sdk/typescript/error-handling) for the full error hierarchy.
