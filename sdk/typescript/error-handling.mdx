...
Error messages are in English (US) by default. See [Localization](/sdk/typescript/utilities#localization) to receive SDK messages in another language.
...
```ts
import { TwoFactorAuthRequiredError, AuthenticationError } from '@cocartheadless/sdk';
...
See [Two Factor Authentication](/sdk/typescript/authentication#two-factor-authentication) for the full flow.
...
See [Disambiguating a Failed Login](/sdk/typescript/authentication#disambiguating-a-failed-login) for the full explanation.
...
See [Legacy Plugin Support](/sdk/typescript#legacy-plugin-support) for the full list of Starter-only methods.
