---
title: "TypeScript SDK: Localization"
sidebarTitle: "Localization"
description: "Translate CoCart TypeScript SDK-internal messages — validation errors, JWT errors, debug logs — independently of your WooCommerce store's language."
'og:title': "SDK Message Localization — CoCart TypeScript SDK"
'og:description': "Register locale catalogs, set a per-client or global locale, and translate CoCart SDK-internal validation, JWT, and debug messages without changing the store's language."
---
...
<Note>
  This section is for contributors to the SDK itself, adding a new built-in locale to `@cocartheadless/sdk` — not something most consumers of the SDK need to do. If you just want your own app to speak a language not yet shipped, register your own catalog with `registerLocale()` (see above) instead.
</Note>

A locale must register all 18 message keys via `registerLocale`:
