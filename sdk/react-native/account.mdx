---
title: "React Native SDK: Account API"
sidebarTitle: "Account API"
description: "Access customer profiles, order history, downloads, and reviews with the CoCart React Native SDK Account API."
'og:title': "Customer Account API — CoCart React Native SDK"
'og:description': "Read and update a logged-in customer's profile, list and fetch orders (including guest order lookup), downloadable files, and submitted reviews from React Native."
---
...
Access it via the `useAccount()` hook or directly through `useCoCart()`.
...
```tsx
import { useAccount } from '@cocartheadless/react-native-sdk';
...
import { useAccount, CoCartError } from '@cocartheadless/react-native-sdk';
...
```tsx
import { useAccount, AuthenticationError, CoCartError } from '@cocartheadless/react-native-sdk';
...
See [Error Handling](/sdk/react-native/error-handling) for the full error hierarchy.
