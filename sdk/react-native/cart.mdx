---
title: "React Native SDK: Cart API"
sidebarTitle: "Cart API"
description: "Add, update, and remove cart items, apply coupons, calculate shipping, and read cart totals with the CoCart React Native SDK."
'og:title': "Cart API — CoCart React Native SDK"
'og:description': "Manage the shopping cart from React Native using useCart and useCartMutations — add items, coupons, shipping, ETag caching, and client-side validation."
---
...
The Cart API handles all shopping cart operations — adding items, updating quantities, applying coupons, and managing the cart state. Access it through the `useCart()` and `useCartMutations()` hooks, or directly via `useCoCart()`.

## Hooks Overview

```tsx
import { useCart, useCartMutations, useCoCart } from '@cocartheadless/react-native-sdk';
...
```tsx
import { ValidationError } from '@cocartheadless/react-native-sdk';
...
See the full Cart API documentation for all available methods (coupons, shipping, fees, cross-sells, etc.) in the [TypeScript SDK docs](/sdk/typescript/cart).
