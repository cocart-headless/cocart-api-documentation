---
title: "PHP SDK: Utilities"
sidebarTitle: "Utilities"
description: "Utility classes in the CoCart PHP SDK — currency formatting and timezone helpers."
'og:title': "Currency & Timezone Utilities — CoCart PHP SDK"
'og:description': "Format WooCommerce prices correctly for any locale using CurrencyFormatter, and convert store timezone dates with TimezoneHelper. Supports all WooCommerce currencies including zero-decimal."
---
...
The SDK includes standalone utility classes for common tasks. These work with data already returned by the API — no extra HTTP requests needed.

## CurrencyFormatter

CoCart returns prices as **smallest-unit integers** — `4599` means $45.99 (4599 cents). `CurrencyFormatter` converts these into human-readable strings.
...
```php
$currency = $response->getCurrency(); // currency_minor_unit = 0
...
2. If `intl` is not available, it falls back to using `currency_prefix`, `currency_suffix`, `currency_decimal_separator`, and `currency_thousand_separator` from the API response.
