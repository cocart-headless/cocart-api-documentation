...
<AccordionGroup>
    ...
    <Accordion title="Performance issues after disabling cache">
        ...
        **Solution**: CoCart already handles selective caching automatically — cart, logout, and store routes receive `no-cache` headers while product routes receive `Cache-Control: public, max-age=3600, stale-while-revalidate=86400`. You should not need to set cache headers manually.

        If you need to adjust the cache duration for product routes, use the built-in filters instead of adding custom header logic. See the [Cache Control](/cache#customization-with-filters) page for details.
    </Accordion>
</AccordionGroup>
...
- [CoCart Load Cart Documentation](/load-cart)
- [CoCart Session Management](/api-reference/v2/sessions/get-sessions)

<Note>
    If you continue experiencing caching issues after implementing these configurations, [contact support](/support) with your host information and error logs.
</Note>
