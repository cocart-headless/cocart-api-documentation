...
<AccordionGroup>
    <Accordion title="Do you offer a free trial?">
        No. But you can try the latest version of CoCart [online for free with the Playground](https://cocartapi.com/try-free-demo/) to see if it's right for you.
    </Accordion>

    <Accordion title="Do you offer refunds?">
        Yes. But before you make a decision to purchase. We recommend that you [try the Playground](https://cocartapi.com/try-free-demo/) first to try the product out at **no cost**.
    ...
    <Accordion title="Do you offer purchasing power parity (PPP)?">
        Yes, local prices are automatically applied to supported countries to make our prices fair for everyone. If a local price is not set for your country, [contact us](mailto:support@cocartapi.com) and we will sort something out.
    </Accordion>
</AccordionGroup>
...
<AccordionGroup>
    ...
    <Accordion title="Are updates free?">
        Plugin updates for the core plugin and any free add-ons will always get free updates. CoCart Starter, Plus and any premium add-on requires a valid license key to receive updates.
    </Accordion>
    ...
    <Accordion title="How well does it work with other plugins?">
        ...
        We do have a [list of WooCommerce extensions](/integrations) that have been tested and are known to work with CoCart.
    </Accordion>
</AccordionGroup>
...
<AccordionGroup>
    ...
    <Accordion title="Why doesn't my guest cart merge into my cart when I log in as an administrator or shop manager?">
        Cart merging on login is intentionally restricted to users with the **customer** or **subscriber** role (or any custom role permitted via the `cocart_user_customer_roles` filter).

        Administrators and shop managers are excluded by design. Because these roles can access any session in the store, automatically merging an arbitrary guest cart into their account could silently overwrite or corrupt session data for other customers they may be managing.

        If you are testing the login merge flow, make sure you are authenticating with a customer-role account rather than the site administrator (user ID 1) or a shop manager. The merge will not trigger for those roles regardless of whether a `cart_key` is supplied alongside the authentication token.
    </Accordion>
